...
//! `System.Runtime`).
//!
//! A [`Uri`] is a `GCHandle`-rooted owner of a managed `System.Uri` object living on the CLR heap; this
...
///
/// Its Rust storage contains only an opaque `GCHandle` token. Cloning creates an independent root
/// for the same immutable managed URI; the wrapper is deliberately not `Copy`.
#[repr(transparent)]
pub struct Uri {
    h: ManagedRef<MUri>,
}
...
impl Uri {
    ...
    pub fn new(uri: &str) -> Self {
        Self {
            h: ManagedRef::from_raw(MUri::new(DotNetString::from(uri).handle())),
        }
    }
...
impl Clone for Uri {
    fn clone(&self) -> Self {
        Self::from_handle(self.h.copy_raw())
    }
}
...
impl core::fmt::Display for Uri {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let s = DotNetString::from_handle(self.h.copy_raw().to_string());
...
impl core::fmt::Debug for Uri {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let s = DotNetString::from_handle(self.h.copy_raw().to_string()).to_rust_string();
...
impl PartialEq for Uri {
    fn eq(&self, other: &Self) -> bool {
        MUri::op_equality(self.h.copy_raw(), other.h.copy_raw())
    }
}
impl Eq for Uri {}
