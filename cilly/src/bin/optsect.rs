...
fn asm_with_fuel(asm: &Assembly, path: &Path, fuel: u32) {
    ...
    asm.verify_for_export()
        .unwrap_or_else(|error| panic!("refusing to export invalid optimized CIL: {error}"))
        .export(path, ILExporter::new(*ILASM_FLAVOUR, false, None));
    ...
    let cfg = cilly::il_exporter::get_default_runtime_config();
