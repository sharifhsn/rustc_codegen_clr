...
| `write_acceptance_artifact_receipt.sh` | Copies a passing oracle's named artifacts into its evidence-owned directory and writes a relative-path SHA-256 receipt, so later cleanup cannot invalidate the evidence. |
| `write_capability_evidence_receipt.sh` | Hash-binds the strict merged report, manifest, every result TSV, every per-oracle receipt, and every copied artifact to source/toolchain/host state. Release receipts reject a dirty source tree. |
