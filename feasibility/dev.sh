...
# that NO cfg flip can supply (mio = target_os arms, socket2 = a crate-internal
# type alias, tokio = a feature gate; see dotnet_overlays/README.md). The overlays
# live ONCE under /work/dotnet_overlays/{mio,socket2,tokio} (REGISTRY.toml lists
# name/version/dir). We auto-redirect any of them present in this project's dep
# graph via a generated `.cargo/config.toml` `paths = [...]` override — the user's
# tracked Cargo.toml is NEVER touched, and `paths` is graph-wide by crate NAME so
# it covers a TRANSITIVE dep too (mio pulled in by tokio). This is the core of the
# future `cargo dotnet`. A `paths` entry whose crate isn't in the graph (or whose
# version doesn't satisfy the locked requirement) is silently ignored by cargo, so
# emitting all overlays is safe; we ALSO parse the lock afterwards and warn loudly
# on a name-match/version-mismatch (the "overlay silently not applied" footgun).
...
"
  ...
  {
    echo '# GENERATED by feasibility/dev.sh apply_overlays — do not hand-edit.'
    echo '# (Phase G: the dotnet_overlays paths-override; future `cargo dotnet`.)'
    echo 'paths = ['
