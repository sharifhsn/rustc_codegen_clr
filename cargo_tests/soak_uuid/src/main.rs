...
//!
//! NO getrandom wiring: uuid's "v4" feature pulls getrandom 0.4 transitively, and the
//! dotnet_overlays/getrandom-0.4 overlay supplies a self-contained `target_os="dotnet"`
//! backend (the PAL CSPRNG) — so `Uuid::new_v4()` just works with no custom symbol/dep.
use uuid::Uuid;
