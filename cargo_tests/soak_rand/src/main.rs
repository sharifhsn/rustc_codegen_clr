...
// NO getrandom wiring: rand -> rand_core -> getrandom 0.2 transitively, and the
// dotnet_overlays/getrandom-0.2 overlay supplies a self-contained `target_os="dotnet"`
// backend (the PAL CSPRNG) — no `custom` feature, no register_custom_getrandom! macro.
