...
impl Namespace {
    ...
    pub fn export_root(&self, out: &mut impl Write, in_mycorrhiza: bool) {
        // This header is emitted here (not hand-added to bindings.rs) so it survives regeneration —
        // spinacz overwrites the whole file every run.
        if in_mycorrhiza {
            writeln!(
                out,
                "//! Generated by `spinacz` (`cargo_tests/spinacz`) — do not hand-edit, rerun spinacz instead.\n\
                 //!\n\
                 //! Every `pub type X = ...RustcCLRInteropManagedClass<ASSEMBLY, CLASS_PATH>` here is the\n\
                 //! SAME Rust type as any other alias (here or hand-written elsewhere in `mycorrhiza`, e.g.\n\
                 //! [`crate::system::MString`]) naming the identical `(ASSEMBLY, CLASS_PATH)` pair — no cast\n\
                 //! needed to move between them, and an `impl` on one applies to both. See the type-identity\n\
                 //! doc on `RustcCLRInteropManagedClass` in `mycorrhiza/src/intrinsics.rs`.\n\
                 //!\n\
                 //! Every reflected type with a resolvable .NET base class gets an `impl From<Derived> for\n\
                 //! Base` below — grep this file for `impl From<` before writing a manual\n\
                 //! `rustc_clr_interop_managed_checked_cast`; `.into()` almost always already exists."
            )
            .unwrap();
        }
