...
// The historical `test/fuzz` and `autodiff` registrations are intentionally absent: those
// fixtures target obsolete custom-MIR or nightly-only APIs and are not current managed oracles.
