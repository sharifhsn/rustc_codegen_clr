...
| Status | Item | What the user gets | Effort | Payoff | Notes / deps |
|---|---|---|---|---|---|
| ✅ | **`#[dotnet_export]` auto-marshal ⚑** | write idiomatic Rust signatures; C# receives typed strings, arrays/containers, nullable primitives, tasks, and exceptions | M–L | ★★★ | Marshals `&str`/`String`/primitives, primitive `Vec<T>`→managed `T[]`, explicit `RustOwnedVec<T>`→`RustVec<T>`, `Task<T>`, primitive `Option<T>`↔`Nullable<T>`, and `Result<T,E>` with explicit `error="exception"`. Proof: `cd_export`, `cd_export_ergonomics`. DTO arrays/try-patterns and managed-reference `Option<T>` remain. |
