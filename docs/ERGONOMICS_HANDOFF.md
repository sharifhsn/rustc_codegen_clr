...
| **Reusable C#→Rust container (Rust)** | `mycorrhiza/src/containers.rs` | `export_rust_containers!()` — emits the size-erased `rcl_vec_*` `#[unsafe(no_mangle)]` core into the invoking cdylib. **Pattern to copy for a new exported container** (e.g. `export_rust_hashmap!`). |
...
- **Add a reusable exported container for C#**: a new `export_*!` macro in `containers.rs` (emit
  `#[unsafe(no_mangle)]` fns into the user crate) + a C# wrapper in `msbuild/RustDotnet.Containers.cs` (or a new
