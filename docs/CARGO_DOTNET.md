...
Raw `extern "C"` declarations generated by `cargo dotnet bindgen` are always a supported escape
hatch; `native_api!` is opt-in. If a facade is requested, malformed function, handle, scoped
callback, or retained-callback policies fail at that named declaration with the observed policy,
the complete required contract, and the raw-declaration alternative. This catches missing status,
success, free, registration, quiescence, threading, and contradictory string/out projections before
the generated wrapper can hide an ownership mistake.
