...
fn write_config(ctx: &Context, paths_lines: &str) -> Result<()> {
    ...
    let pinvoke = [
        ctx.paths.sdk_crates_root.join("rust-dotnet-pinvoke"),
        ctx.paths.sdk_crates_root.join("crates/rust-dotnet-pinvoke"),
    ]
    .into_iter()
    .find(|path| path.is_dir());
    let manifest = ctx.crate_dir.join("Cargo.toml");
    let mut patches = Vec::new();
    for (name, path) in [
        ("mycorrhiza", Some(mycorrhiza.clone())),
        ("dotnet_macros", Some(dotnet_macros.clone())),
        ("rust-dotnet-pinvoke", pinvoke),
    ] {
        let Some(path) = path.filter(|path| path.is_dir()) else {
            continue;
        };
        // A development checkout must override registry SDK dependencies, but adding patches for
        // absent packages or packages already selected by a direct path dependency is noisy and,
        // with --locked, can force an otherwise unnecessary lockfile rewrite.
        if crate::metadata_inputs::manifest_declares_dependency(&manifest, name)?
            && !crate::metadata_inputs::manifest_uses_direct_path_dependency(&manifest, name)?
        {
            patches.push(format!("{name} = {{ path = {path:?} }}"));
        }
    }
    let sdk_patches = if patches.is_empty() {
        String::new()
    } else {
        format!("[patch.crates-io]\n{}\n", patches.join("\n"))
    };
    // Keep SDK crates in the explicit path set as well as the patch table. Cargo can report a
    // patch as unused when a fresh consumer lockfile has already selected a path/source variant
    // of the same package; the graph-wide paths override removes that installed-mode ambiguity.
    ...
    for path in [
        ...
        if path.is_dir() {
            all_paths.push_str(&format!("    {},\n", toml_path_literal(&path)));
        }
    }
    all_paths.push_str(paths_lines);
    let config = format!(
...
pub(crate) fn remove_legacy_generated_config(crate_dir: &Path) -> Result<()> {
    ...
    if text.starts_with("# GENERATED by cargo dotnet (overlays::apply) — do not hand-edit.\n")
        ...
        || text
            .starts_with("# GENERATED by feasibility/dev.sh apply_overlays — do not hand-edit.\n")
    {
        fs::remove_file(&path)
...
pub fn apply(ctx: &Context) -> Result<()> {
    ...
    write_config(ctx, "")?;
    let lock_path = ctx.workspace_root.join("Cargo.lock");
...
mod tests {
    ...
    #[test]
    fn removes_every_known_generated_crate_config_but_preserves_user_config() {
        ...
        for header in [
            "# GENERATED by cargo dotnet (overlays::apply) — do not hand-edit.\n",
            "# GENERATED by feasibility/cargo-dotnet (apply_overlays) — do not hand-edit.\n",
            "# GENERATED by feasibility/dev.sh apply_overlays — do not hand-edit.\n",
        ] {
            fs::write(&config, format!("{header}[build]\ntarget = 'stale'\n")).unwrap();
            remove_legacy_generated_config(temp.path()).unwrap();
            assert!(!config.exists(), "generated config with header {header:?}");
        }
