...
cfg_if! {
    if #[cfg(any(target_os = "linux", target_os = "android"))] {
        #[path = "linux.rs"]
        mod imp;
    } else if #[cfg(target_os = "dotnet")] {
        // DOTNET PAL: the x86_64-unknown-dotnet target sets target-family=["unix"]
        // (so unmodified upstream mio picks its epoll selector). Without this arm the
        // `cfg(unix)` branch below would select `unix.rs` — the raw pthread_mutex /
        // pthread_cond parker — which has no .NET libc and fails to compile (E0425:
        // pthread_mutex_t, pthread_cond_wait, ...). .NET has no pthread, so route to
        // upstream's own portable `generic.rs` parker (a spin-lock over AtomicBool +
        // thread::yield_now + Instant — ZERO libc, all on the dotnet PAL). MUST precede
        // the `unix` arm. This unblocks parking_lot/lock_api consumers (dashmap, ...).
        #[path = "generic.rs"]
        mod imp;
    } else if #[cfg(unix)] {
        #[path = "unix.rs"]
