...
// DOTNET PAL ARM — the ONLY net-new socket2 source line (otherwise byte-identical
// to crates.io socket2 0.6.4). target_os="dotnet" (target_env="") matches NEITHER
// the gnu/uclibc-64/android arm above NOR the musl/bsd/aix arm below, so `IovLen`
// would be undefined (E0412 at set_msghdr_iov, line ~771). size_t==usize on
// x86_64 (matches the Linux gnu arm the POSIX shim's msghdr layout mirrors); this
// alias is only used by sendmsg/recvmsg, which the dotnet build never CALLS — it
// just needs to type-check. No libc widening can supply a crate-internal alias.
#[cfg(target_os = "dotnet")]
type IovLen = libc::size_t;
