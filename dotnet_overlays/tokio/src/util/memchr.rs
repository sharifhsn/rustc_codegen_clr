...
// DOTNET PAL ARM: route os=dotnet to the pure-Rust fallback (exclude it from the
// libc arm). The dotnet libc face does not model `memchr` (a pure mem function),
// and the trivial iterator scan is correct + needs no libc symbol. ONE extra
// `not(target_os = "dotnet")` on each arm; otherwise byte-identical.
#[cfg(not(all(unix, not(target_os = "dotnet"), feature = "libc")))]
fn memchr_inner(needle: u8, haystack: &[u8]) -> Option<usize> {
    haystack.iter().position(|val| needle == *val)
}

#[cfg(all(unix, not(target_os = "dotnet"), feature = "libc"))]
fn memchr_inner(needle: u8, haystack: &[u8]) -> Option<usize> {
