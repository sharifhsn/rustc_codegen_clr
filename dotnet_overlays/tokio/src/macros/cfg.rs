...
macro_rules! cfg_net_unix {
    ($($item:item)*) => {
        $(
            // DOTNET PAL ARM: tokio's `net::unix` (UnixStream/Listener/Datagram +
            // unix::pipe + AsyncFd) is built under `cfg(all(unix, feature="net"))`.
            // The global target-family=["unix"] flip makes that TRUE for
            // target_os="dotnet", but tokio's unix surface needs mio's uds + pipe
            // (the latter is `IoSource<std::fs::File>`, and dotnet std fs::File is
            // not fd-backed — the deferred eventfd-waker wall) plus libc stat/
            // fstat/O_*/memchr/ucred that the dotnet libc face does not model. The
            // I/O-tokio goal (pal_tokio_net) is TCP-only and never touches AF_UNIX,
            // so EXCLUDE dotnet from this gate (the ONLY net-new tokio source edit;
            // otherwise byte-identical to crates.io 1.52.3). Re-enabling tokio's
            // AF_UNIX for dotnet needs mio uds + fd-backed fs::File — an explicit
            // follow-up (LIBC_SHIM_SCOPE §2.2). The cfg_net (TCP/UDP) gate is
            // untouched, so TcpStream/TcpListener compile + run.
            #[cfg(all(unix, not(target_os = "dotnet"), feature = "net"))]
