...
#[cfg(feature = "std")]
mod term;
#[cfg(all(unix, not(target_arch = "wasm32"), not(target_os = "dotnet"), feature = "std"))] // DOTNET PAL: use the dummy terminal backend because the CLR PAL has no Unix fd terminal ABI.
mod unix_term;
#[cfg(feature = "std")]
mod utils;
#[cfg(all(feature = "std", any(target_arch = "wasm32", target_os = "dotnet")))] // DOTNET PAL: the wasm-style no-op terminal backend is the portable CLR fallback.
mod wasm_term;
