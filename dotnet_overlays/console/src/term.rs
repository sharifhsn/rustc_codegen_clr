...
#[cfg(any(all(unix, not(target_os = "dotnet")), all(target_os = "wasi", target_env = "p1")))]
...
impl TermFeatures<'_> {
    ...
    pub fn family(&self) -> TermFamily {
        ...
        #[cfg(all(unix, not(target_arch = "wasm32"), not(target_os = "dotnet")))]
        {
            TermFamily::UnixTerm
        }
        #[cfg(any(target_arch = "wasm32", target_os = "dotnet"))]
        {
            TermFamily::Dummy
        }
    }
