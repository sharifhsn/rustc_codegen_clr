...
cfg_if! {
    // DOTNET PAL: entropy via the BCL CSPRNG (rcl_dotnet_random_fill). FIRST arm so it
    // pre-empts the late `feature = "custom"` arm regardless of feature unification —
    // dotnet needs NO `custom` feature/macro. dotnet is a unique os, so being first
    // cannot change any other target's selection.
    if #[cfg(target_os = "dotnet")] { // DOTNET PAL
        #[path = "dotnet.rs"] mod imp; // DOTNET PAL
