...
cfg_if! {
    if #[cfg(getrandom_backend = "custom")] {
        ...
        pub use extern_impl::*;
    } else if #[cfg(target_os = "dotnet")] { // DOTNET PAL: entropy via the BCL CSPRNG (rcl_dotnet_random_fill). Below the getrandom_backend opt-in arms so an explicit cfg still wins; above the per-os arms + compile_error! tail. dotnet is a unique os, so ordering vs other target_os arms is irrelevant.
        mod dotnet; // DOTNET PAL
        pub use dotnet::*; // DOTNET PAL
